...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<Map<String, dynamic>> fetchServerResources(String uuid) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'current_state': data['current_state'] ?? 'Unknown',
          // Add current state
          'cpu_usage': resources['cpu_absolute'] ?? 0,
          'memory_usage': (resources['memory_bytes'] / (1024 * 1024))
              .toStringAsFixed(2),
          // Convert bytes to MB
          'disk_usage': (resources['disk_bytes'] / (1024 * 1024))
              .toStringAsFixed(2),
          // Convert bytes to MB
        };
      } else {
          ...
          for (var serverData in data) {
            ...
            if (existingServerIndex != -1) {
              ...
              servers.add({
                ...
                'ip': allocation?['ip'] ?? 'carrot.melonhost.cz',
                // Replace IP with hardcoded value
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          if (!isDesktop)
            DrawerHeader(
              decoration: BoxDecoration(color: Color(0xFFBB86FC)),
                  ...
                  child: Card(
                    ...
                    shape: RoundedRectangleBorder(
                      ...
                      BorderRadius.circular(
                          12), // Match container's rounded corners
                    ),
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 6),
                          // Reduced space between title and details
                          Text(server['description'],
                              style:
                              TextStyle(
                                  fontSize: 14, color: Color(0xFF808080))),
                          SizedBox(height: 4),
                          // Reduced space between details lines
                          Text(
                              'CPU Usage: ${server['cpu_usage']}% / ${server['cpu']}%',
                              style:
                              TextStyle(
                                  fontSize: 14, color: Color(0xFF808080))),
                          Text(
                              'RAM Usage: ${server['memory_usage']} MB / ${server['memory']} MB',
                              style:
                              TextStyle(
                                  fontSize: 14, color: Color(0xFF808080))),
                          Text(
                              'Disk Usage: ${server['disk_usage']} MB / ${server['disk']} MB',
                              style:
                              TextStyle(
                                  fontSize: 14, color: Color(0xFF808080))),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Melodactyl Dashboard'),
        backgroundColor: Color(0xFF121212),
        // Set the background color to match scaffold
        surfaceTintColor: Colors.transparent,
        // Disable Material 3 surface tint effect
        scrolledUnderElevation: 0, // Disable elevation change when scrolled under
      ),
      backgroundColor: Color(0xFF121212),
      body: Row(
        children: [
          if (isDesktop) buildSidebar(isDesktop: true),
      ...
      drawer: isDesktop ? null : Drawer(
          child: buildSidebar()), // Drawer only for mobile devices
    );
  }
}
